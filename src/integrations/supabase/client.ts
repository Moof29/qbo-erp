...
const getClient = () => {
  ...
  if (devMode) {
    ...
    // For REST requests, we need to add custom headers for development mode
    // Using the addHeader method which is the proper way to modify headers
    client.supabaseUrl; // This is just to prevent TypeScript errors by accessing a property
    
    // Add dev user ID to all requests to bypass RLS policies
    const headers = { 'x-dev-user-id': devUserId };
    
    // Apply headers to all future requests
