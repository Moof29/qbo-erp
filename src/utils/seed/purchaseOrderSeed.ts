...
export const seedDummyPurchaseOrders = async (): Promise<SeedResult> => {
  try {
...
export const checkPurchaseOrdersExist = async (): Promise<boolean> => {
  try {
    // Get current user and organization
    const { data: { user } } = await supabase.auth.getUser();
...
export const seedIfEmptyPurchaseOrders = async (): Promise<SeedResult> => {
  try {
    const hasPurchaseOrders = await checkPurchaseOrdersExist();
    if (!hasPurchaseOrders) {
      return await seedDummyPurchaseOrders();
    }
    return { 
      success: true, 
      count: 0, 
      message: "Purchase orders already exist" 
    };
  } catch (error: any) {
    console.error("Error in seedIfEmptyPurchaseOrders:", error);
